...
- 対応: Godot 4.3 以降、Windows。CLAP プラグイン(Surge XT 等)の音源のトラックはゲームでは鳴らない
  (`get_warnings()` に出る。人間に Glaux でそのトラックを「音声にする(フリーズ)」してもらう)
- **ゲームを書き出すには「プロジェクト設定 → プラグイン」で Glaux を有効にしておく**(曲のファイルを .pck に入れる。
  無効だと書き出したゲームで `load_song` が失敗する)。曲を `res://` の外や `.glaux` 以外の名前のフォルダに置かない
