...
## 0.2.0(2026-09-26)ループ・展開の切り替え・トラックの音量、音の処理をアプリ 0.2.0 と揃える、Linux 版

### 新しい機能(場面に合わせて曲を組み替える)

曲のファイルは変えずに、ゲームの場面に合わせて鳴らし方を変えられるようになりました。使い方は `README.md` の
「ループ・展開の切り替え・トラックの音量」と `AI_GUIDE.md` の「3-7b」にあります。

- `set_loop_section(name)` / `set_loop(from, to)` / `clear_loop()` / `is_looping()`: マーカーの区間や秒の範囲を繰り返す
- `queue_section(name, at = "bar", loop_it = false)`: マーカーの区間へ、次の拍・次の小節の頭・今の区間の終わりで
  切り替える予約(`cancel_queued_section()` / `get_queued_section()`)。切り替えはサンプル単位で正確です
- `set_track_volume_db(track, db, fade = 0.0)` / `get_track_volume_db(track)`: 楽器の足し引き(フェード付き)
- シグナル `jumped(from, to)`: ループの折り返し・切り替えが聞こえた。飛ぶ位置の先の拍・マーカーは知らせません
- ループ中・切り替えの後は、`get_song_time()` が飛んだ所で戻ります(単調増加ではなくなります)

### 音の処理(Glaux 0.2.0 と同じ音になります)

- EQ は新しい方式(SVF)になり、ハイパス・ローパスが使えます。コンプはなだらかに効き始めるニー、peak / rms の検出、
  検出側のハイパスに対応しました
- リバーブは 8 本の遅延線(FDN)になり、プリディレイとプレートの響きが使えます
- ステレオのトラック(ステレオの音声・バス)のパンは、振った側が大きくならないバランス型になりました
- 新しい内蔵エフェクト: マルチバンドコンプ・トランジェントシェイパー・リミッタ・ステレオの幅・ダイナミック EQ・
  畳み込みリバーブ(曲に取り込んだ響き)・共鳴抑制・仮想低音
- 歪み系の折り返し雑音を減らし、サンプラー・SoundFont を高く鳴らしたときの折り返し雑音も抑えました。
  ディレイ・コーラス・テープの揺れ、撥弦(pluck)の高い音の音程が正確になりました
- 音量・パンとマスター音量のオートメーションが、約 5ms でなめらかにつながります
- 今までの曲も鳴りますが、EQ・コンプ・リバーブを使っている曲は少し音が変わります

### そのほか

- Linux(x86_64)版の `.so` を同梱しました(macOS はビルドの手順だけ用意していて、未確認です)
- それ以外の API・シグナル・設定は今までどおりです。`.dll` / `.so` を差し替えるだけで使えます
