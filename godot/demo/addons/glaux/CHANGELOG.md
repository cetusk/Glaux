...
## 0.1.1(2026-09-26)外してあるエフェクトを鳴らさない / 版の番号をアプリにそろえる
...
- **この版から、アドオンの版の番号を Glaux アプリの版にそろえました**(アプリ v0.1.1 にはアドオン 0.1.1)。
  以前の 0.3.0 は、アプリ v0.1.0 と一緒に公開したもので、ここでは 0.1.0 と呼び直しています

---

## 0.1.0(2026-09-25)最初の公開版(Glaux v0.1.0 と一緒に公開。当時の番号は 0.3.0)
...
公開前(アプリ v0.1.0 より前)に、アドオンだけを番号を付けて更新していたときの記録です。

#### 開発版 0.2.1(2026-09-25)エディタのノードに Glaux のアイコン
...
- 機能・メソッドの変更はありません。更新のしかたは開発版 0.2.0 と同じです(配布物で `addons/glaux/` を丸ごと置き換える)

#### 開発版 0.2.0(2026-09-25)効果音を WAV なしで鳴らし、音程を BGM のコードに合わせる
...
2. できた `godot\dist\glaux-godot-addon.zip` の中身で、ゲーム側の `addons\glaux\` を**丸ごと置き換える**
   (DLL と説明書が同じ版にそろいます)
3. Godot のエディタを開き直す
4. 更新できたかの確認: スクリプトで `music.has_method("play_note")` が true なら開発版 0.2.0 以降です

#### 開発版 0.1.0(2026-09-24)初版: 曲の再生と、拍・マーカー・ノートへの同期
