...
## 0.1.3(2026-09-26)音の処理をアプリ 0.1.3 と揃える(EQ・コンプ・遅延・音量の変化)

### 要点

- Glaux 0.1.3 と同じ音の処理になります
  - EQ: 新しい方式(SVF)になり、ハイパス・ローパスが使えます。周波数やゲインを動かしても「ジジッ」と鳴りません
  - コンプ: なだらかに効き始めるニー(既定 6dB)、peak / rms の検出、検出側のハイパスに対応しました
  - ディレイ・コーラス・テープの揺れと、撥弦(pluck)の高い音の音程が正確になりました
  - トラックの音量・パンとマスター音量のオートメーションが、約 5ms でなめらかにつながります
- 今までの曲も鳴りますが、EQ とコンプを使っている曲は少し音が変わります(コンプはニーの分だけ柔らかくなります)
