...
## 0.1.2(2026-09-26)エフェクトの分岐・合流(Glaux の「線でつなぐ」ノード表示)に対応

### 要点

- Glaux 0.1.2 のミキサーで、エフェクトを線で分けたり混ぜたりした曲(`project.json` の `fx_links`)を、アプリと同じ音で鳴らします
  (原音 + リバーブの並列、線ごとの音量など)。入力から出口まで線でたどれないエフェクトは鳴りません
- CLAP エフェクトは今までどおりゲームでは掛かりません(つながりを保つため、素通しとして扱います)
- API・シグナル・設定の変更はありません。`.dll` / `.so` を差し替えるだけで使えます

---

## 0.1.1(2026-09-26)外してあるエフェクトを鳴らさない

### 要点

- Glaux 0.1.1 のミキサーで「線から外して取っておいた」エフェクト(`project.json` の effects で `"parked": true`)を、
  アプリと同じく鳴らさないようにしました。0.1.0 までの拡張では、外してあるエフェクトも通して鳴っていました
- API・シグナル・設定の変更はありません。`.dll` / `.so` を差し替えるだけで使えます
- アドオンの版の番号は、Glaux アプリの版と同じです(アプリ v0.1.1 と一緒に使うのはアドオン 0.1.1)

---

## 0.1.0(2026-09-25)最初の公開版(Glaux v0.1.0 と一緒に公開)

書き出したゲームで曲を読めるように / CLAP のトラックは「音声にする」で
...
公開前(アプリ v0.1.0 より前)の、開発中の変更の記録です。

#### 2026-09-25 エディタのノードに Glaux のアイコン
...
- 機能・メソッドの変更はありません。更新のしかたは次の「効果音を WAV なしで鳴らし…」と同じです(配布物で `addons/glaux/` を丸ごと置き換える)

#### 2026-09-25 効果音を WAV なしで鳴らし、音程を BGM のコードに合わせる
...
2. できた `godot\dist\glaux-godot-addon.zip` の中身で、ゲーム側の `addons\glaux\` を**丸ごと置き換える**
   (DLL と説明書が同じ版にそろいます)
3. Godot のエディタを開き直す
4. 更新できたかの確認: スクリプトで `music.has_method("play_note")` が true ならこの変更が入っています

#### 2026-09-24 初版: 曲の再生と、拍・マーカー・ノートへの同期
