...
ゲーム側(Glaux の曲を使っているゲーム)の要望: 効果音を WAV にせず、音程を BGM のキー・コードに合わせて鳴らしたい(入力の瞬間と、拍に合わせる
音の両方)。
...
- アプリのアイコン `app/src-tauri/icons/icon.ico`: 40px 以下は顔のみ版(小さくても目元が分かる)、
  48px 以上は白フチの全身版(暗いタスクバーでも形が見える)。`icon.png` は白フチの全身版 512px。
...
- 生成は `scripts/make_icons.py`(Pillow が要る。元の PNG は変更しない)。2026-09-25 に「小さいアイコンがぼやける」との
  報告で作り直した: 40px 以下は顔のみ版を余白 1/16 に詰め(ガイドの 1/8 は配置の目安。この大きさでは読みやすさを優先)、
  4 倍の大きさを経て 2 段階で縮めてから小さい半径でアンシャープ。ICO には Windows が表示倍率ごとに使う
  16/20/24/30/32/36/40/48/60/64/72/80/96/128/256px をすべて入れた(無いサイズは拡大・縮小されてぼやける)
- タスクバー(2026-09-25): Tauri はウィンドウのアイコンに ICO の**最初の 1 枚**だけを使う(tauri-codegen の
  `CachedIcon::new_ico` が `entries()[0]`)。Pillow の ICO は小さい順なので 16px の顔が使われ、Windows がタスクバーの
  大きさへ拡大してぼやけていた。対策: (1) ICO を自前で大きい順(256px が先頭、各画像は PNG)に書く、
  (2) タスクバー用に 24px × 表示倍率(24/30/36/48/60/72/96px)の RGBA 生データを `app/src-tauri/icons/taskbar/` に置き、
  `main.rs` の `taskbar_icon` で起動時と `ScaleFactorChanged`(表示倍率の違うモニターへ移動)で設定する
- 余白を最小に(2026-09-25、ユーザーの指示でガイドの推奨余白より優先): アイコンは見える図形の端で切り、正方形に要る分だけ
  透明を足す。元の PNG には背景を抜いたときのほぼ透明なもや(不透明度 1〜8)が広く残っていて、以前はそれを図形に含めて
  余白が大きく残っていた(顔のみ版の「左端がキャンバスに接している」も、このもやの見誤り)。不透明度 8 以下は消してから切る。
  README のロゴは白背景の横組みを文字とフクロウの端で切り、幅 960px の `docs/images/glaux-logo.png` に(原本は 1774px・870KB)
