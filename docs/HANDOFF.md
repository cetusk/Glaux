...
7. **ボリューム/パンのオートメーション対応**(2026-09-22 追加。ユーザー要望)
   - モデルとコマンド(`AutomationLane` / `set_automation_points`)は glaux-core に**既にある**が、
     **エンジンが適用していない**(AI が書いても音に反映されない)
   - 第 1 段: エンジン対応(track/volume_db・track/pan のレーンをサンプル位置の
     ブレークポイント列に焼き込み、レンダラで区分線形補間)→ これだけで
     **AI による連続的なミキシング**(フェードイン、ビルドアップの音量カーブ等)が即使える
   - 第 2 段: 人間用 UI。設計案: トラックヘッドに「オートメーション」展開ボタン →
     トラック下にレーンが開き、ピアノロールと同様の Canvas でカーブ表示・
     クリックで点追加・ドラッグで移動・右クリック削除(すべて set_automation_points 経由)

8. **途中の拍子変更のサポート**(2026-09-22 追加。プログレ対応)
   - モデル(`time_sig_map`)とコマンド(`set_time_sig`)は既にあり、AI は書ける
   - 未対応なのは **UI のグリッド**(Timeline / PianoRoll とも先頭の拍子で等間隔描画)と
     小節ナビゲーション。拍子イベントを考慮した「tick ↔ 小節」変換関数を共有化して
     グリッド・ルーラー・スナップを作り直す必要がある
