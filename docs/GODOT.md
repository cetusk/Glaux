...
- 使えない: CLAP プラグイン(Surge XT 等)。その音源のトラックは無音になり、`get_warnings()` に理由が出ます。
  ゲームで使うには、そのトラックを音声にして音声クリップに置き換えます。Glaux にはまだトラック単体を
  書き出す機能が無いので、今は次の手順になります(専用の機能は今後の候補です):
  1. Glaux でそのトラックだけをソロにして WAV に書き出す(上部の「⬇ WAV」。プロジェクトの `export/` にできる)
  2. 音声トラックを作り、書き出した WAV を曲頭(0 小節目)に取り込む
  3. 元の CLAP のトラックはミュートするか消す

## 6. 注意事項

### ビルドと DLL の差し替え

- Rust 1.94 以上が必要です。古いと `godot` クレートのビルドで止まります(`rustup update` で更新)
- **DLL を作り直すときは、Godot のエディタを閉じてから**ビルドしてください。エディタが DLL を開いたままだと
  `build_windows.bat` の上書きコピーが失敗することがあります(「別のプロセスが使用中」など)
- 自分のゲームに入れている場合は、作り直した `glaux_godot.dll` をゲーム側の `addons/glaux/bin/` にも
  コピーし直す必要があります(デモのフォルダにしか置かれません)

### 曲のファイル

- **Glaux で曲を直したら、ゲーム側のフォルダへコピーし直してください。** 自動では同期しません
...
- 重いと感じたら、重いトラックを音声クリップに置き換えると軽くなります(手順は 5 章の CLAP の場合と同じ)
...
- 初回の読み込みなどで出る「Native class "GlauxPlayer" not found」: 拡張が読み込まれる前にスクリプトを
  調べたためで、実行には影響しません
- `load_song` のときの「CLAP プラグインのため、ゲームでは鳴りません」: 5 章のとおりです。
  `get_warnings()` でも確認できます

### 未確認のこと

- **ゲームを書き出した(エクスポート)後の読み込み**: 曲を読むには `project.json` と WAV が .pck に入っている
  必要があります。書き出しの設定手順は確認でき次第ここに追記します(エディタ上での実行は確認済み)
- 動作確認済み: Windows(デモ)、Linux(ヘッドレス)。macOS は未確認
