...
| 40 | ミキサー画面・レベルメーター・エフェクトの並べ替え(`move_effect` は既知の未実装) | パンはオートメーションのレーンからだけ。`transport_state` にトラックのメーターなし | 下部パネルに「ミキサー」タブ。トラックごとのピークをアトミックに。`move_effect` コマンド | L | 済(ヘッダーでタイムラインとミキサーを切り替え。上 = 縦の列とメーター、下 = エフェクトのノード表示とエフェクトのプリセットの棚。`move_effect` / `set_effect_prop`) |
