...
- **音源とエフェクト** — 内蔵シンセ 7 種(シンセ・FM・ウェーブテーブル・ドラム・撥弦・サンプラー・SoundFont)、エフェクト 13 種(EQ・コンプ・マルチバンドコンプ・トランジェントシェイパー・リミッタ・ステレオの幅・リバーブ・アンプなど)、CLAP プラグイン(Surge XT など)
...
| 分析 | `analyze_audio` `compare_mix` `analyze_harmony` `analyze_rhythm` `analyze_beats` `analyze_sound` `compare_sounds` `match_sound` |
