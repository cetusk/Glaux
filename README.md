...
![タイムラインと AI への指示・履歴](docs/images/screenshot-timeline.png)
...
- **音源とエフェクト** — 内蔵シンセ 7 種(シンセ・FM・ウェーブテーブル・ドラム・撥弦・サンプラー・SoundFont)、エフェクト 17 種(EQ・ダイナミック EQ・共鳴抑制・仮想低音・コンプ・マルチバンドコンプ・トランジェントシェイパー・リミッタ・ステレオの幅・リバーブ・畳み込みリバーブ・アンプなど)、CLAP プラグイン(Surge XT など)
...
![ミキサーとエフェクトのノード表示](docs/images/screenshot-mixer.png)
...
| 分析 | `analyze_audio` `compare_mix` `master_mix` `analyze_harmony` `analyze_rhythm` `analyze_beats` `analyze_sound` `compare_sounds` `match_sound` `refine_by_words` |
