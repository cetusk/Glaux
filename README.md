...
- **エフェクト 6 種**: eq / compressor / reverb / distortion / **amp(ギターアンプシミュ)** / sidechain(EDM のポンピング)。音量・パンのオートメーション、途中のテンポ・拍子変更にも対応
- **人間の編集**: ピアノロール(複数選択・コピペ・3 連符・奏法 M/S/A/V/B)、ドラムキット図・**ギター/ベースのフレット盤**からの打ち込み、音作りビュー(つまみ・エフェクトチェーン・プリセット)、ループ再生、小節範囲を指定した AI への指示(マスク)
- **奏法(アーティキュレーション)**: ブリッジミュート・スタッカート・アクセント・ビブラート・チョーキングをノート単位で。楽器ごとに効き方が変わります
