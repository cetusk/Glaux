...
(背景ごとの版、反転・引き伸ばし・影や発光の追加をしない)。指定色は Dark `#142332` / Turquoise `#25BDB1` / White。
ただし表示用に生成する画像(アイコン・README のロゴ)は、ユーザーの指示で**余白を最小**にしている(ガイドの推奨余白より優先)。
生成は `scripts/make_icons.py`(`assets/` の原本は変えない)。
