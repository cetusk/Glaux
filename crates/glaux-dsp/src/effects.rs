...
impl EffectState {
    ...
    /// ステレオ 1 サンプル処理。
    pub fn process(&mut self, p: &EffectParams, l: f32, r: f32) -> (f32, f32) {
        match p {
            EffectParams::Eq(eq) => {
...
pub fn bake_effect(effect: &glaux_core::Effect, sample_rate: f32) -> Option<EffectParams> {
...
mod tests {
    ...
    fn eq_low_cut_reduces_low_and_keeps_high() {
        let p = bake_effect(&effect("eq", &[("low_gain_db", -12.0)]), 48_000.0).unwrap();
        let low = rms_through(&p, 100.0, 9600);
        let high = rms_through(&p, 4000.0, 9600);
        let flat = bake_effect(&effect("eq", &[]), 48_000.0).unwrap();
    ...
    fn compressor_reduces_loud_signal() {
        let p = bake_effect(
            &effect("compressor", &[("threshold_db", -20.0), ("ratio", 8.0)]),
            48_000.0,
        )
        .unwrap();
    ...
    fn reverb_produces_tail() {
        let p = bake_effect(&effect("reverb", &[("mix", 0.5)]), 48_000.0).unwrap();
        let mut state = EffectState::default();
        state.ensure_kind(&p);
        // インパルスを入れて、その後の無音区間に残響が出るか
        let (_, _) = state.process(&p, 1.0, 1.0);
        let mut tail = 0.0f32;
        for _ in 0..48_000 {
            let (l, r) = state.process(&p, 0.0, 0.0);
    ...
    fn unknown_effect_is_none_and_catalog_resolves() {
        assert!(bake_effect(&effect("no_such_fx", &[]), 48_000.0).is_none());
    ...
    fn ensure_kind_resets_state_between_kinds() {
        let rv = bake_effect(&effect("reverb", &[("mix", 1.0)]), 48_000.0).unwrap();
        let mut state = EffectState::default();
        state.ensure_kind(&rv);
        state.process(&rv, 1.0, 1.0);
        // 別種に切り替え → リバーブへ戻してもバッファはリセット済み
        let eq = bake_effect(&effect("eq", &[]), 48_000.0).unwrap();
        state.ensure_kind(&eq);
        state.ensure_kind(&rv);
        let mut tail = 0.0f32;
        for _ in 0..4800 {
            let (l, _) = state.process(&rv, 0.0, 0.0);
            tail = tail.max(l.abs());
        }
